...
def count(founded_classes,im0):
    ...
    y2 = 70
    display = im0.copy()                                                  
    cv2.putText(im0, f"{founded_classes}" ,(x1 + 10, y1 + 35), cv2.FONT_HERSHEY_SIMPLEX,0.7, (255, 255, 0), 2)
    cv2.addWeighted(im0, 0.7, display, 1 - 0.7, 0, im0)
def detect(save_img=False):
    time_stamp, source, weights, view_img, save_txt, imgsz, trace = opt.timestamp ,opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace
    save_img = not opt.nosave and not source.endswith('.txt')  # save inference images
    webcam = source.isnumeric() or source.endswith('.txt') or source.lower().startswith(
        ('rtsp://', 'rtmp://', 'http://', 'https://'))
    save_dir = Path(increment_path(Path(opt.project) / time_stamp, exist_ok = True))  # increment run
...
if __name__ == '__main__':
    ...
    parser.add_argument('--timestamp', default = "2", type = str,
